...
contract ROAD_Staking is Ownable {
    ...
    // Owner has to set Road's "Token" and "LP_token" address
    ...
    */
    ...
    // User will enter the address to calculate its Road Points according to 1 DAY.
    function RPcalculator(address addr) public view returns(uint256){

    uint256 remainingenergy;

    if(User[addr].Deposit_time!=0) {
       uint256 reward = ((block.timestamp.sub(User[addr].Deposit_time)).div(time)).mul(User[addr].Total_Amount.mul(ROAD_Perecent));
    ...
    */
    ...
    if(UserLP[addr].Deposit_time!=0) {
        uint256 reward = ((block.timestamp.sub(UserLP[addr].Deposit_time)).div(time)).mul((UserLP[addr].Total_Amount.mul(Lppercent)));
        remainingenergy= (reward.div(1E18)).sub(UserLP[addr].redeemedRP);    
    }
    return remainingenergy;
    }

    // Usrs call this function to store its Road_points in balances mapping
    ...
    */
    ...
    NFTcontract=_add;
    emit NFTAddress(msg.sender, NFTcontract);
    }
    ...
    emit LP_Lock_Time(msg.sender, LPlocktime);
    }
    ...
    time = _epoch;
    emit epochTime(msg.sender, time);
    }


}

//  TOKEN:      0x0C8538E68d598Ad5FC75b5F16A96A4187D47A802
// LP-TOKEN:    0xBbd36739254fcD5062f6fFd863522D9404A9c920